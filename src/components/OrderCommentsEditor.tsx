...
export default function OrderCommentsEditor({ value, onChange, className }: OrderCommentsEditorProps) {
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      ...
      <div className="relative min-h-[320px] rounded-lg border border-slate-200 bg-white p-4 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/40">
        {isEmpty && (
        ...
        <div
          ref={editorRef}
          className="min-h-[280px] w-full outline-none"
          contentEditable
          spellCheck
          onInput={handleInput}
        />
      ...
      <p className="text-xs text-muted-foreground">
        This note will be stored with the order record and can include bold text, colors, and highlights.
      </p>
    </div>
  );
}
