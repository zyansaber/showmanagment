...
import { Mail, History, Send, Eye } from 'lucide-react';
...
const buildDigestPdfDataUri = async (args: {
  ...
  const barcodeX = right - Math.min(215, barcode.width) - 24;
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-[11px] text-slate-500">
                          ...
                          <th className="px-2 py-1 text-left font-medium">Confirmation</th>
                        </tr>
                      </thead>
                      <tbody>
                        {digest.orders.length ? (
                          digest.orders.map((order, idx) => (
                            <tr key={`${order.orderKey}-${idx}`} className="border-t first:border-t-0 border-slate-100 odd:bg-white even:bg-slate-50/40">
                              <td className="px-2 py-1 align-top font-semibold text-slate-900 whitespace-nowrap">#{order.dealNo}</td>
                              <td className="px-2 py-1 align-top whitespace-nowrap">{order.showName || 'Unknown show'}</td>
                              <td className="px-2 py-1 align-top">{order.customerName || '-'}</td>
                              <td className="px-2 py-1 align-top">✓</td>
                            </tr>
                          ))
                        ) : (
                          <tr>
                            <td className="px-2 py-2 text-xs text-slate-500" colSpan={4}>No pending orders</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
