        ...
        <CardContent className="space-y-4">
          {sortedDigests.length === 0 ? (
            ...
            sortedDigests.map((digest) => (
              <div key={digest.member.key} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-lg border border-slate-200">
                    <div className="border-b bg-slate-50 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-600">Orders</div>
                        ...
                        <tbody>
                          {digest.orders.length ? (
                            digest.orders.map((order, idx) => (
                              <tr key={`${order.orderKey}-${idx}`} className="border-t border-slate-100 odd:bg-white even:bg-slate-50/40">
                                <td className="px-3 py-2 font-semibold text-slate-900">{order.dealNo}</td>
                                <td className="px-3 py-2">{order.showName || 'Unknown show'}</td>
                                <td className="px-3 py-2">{order.customerName || '-'}</td>
                              </tr>
                            ))
                          ) : (
                            <tr>
                              <td className="px-3 py-3 text-sm text-slate-500" colSpan={3}>No pending orders</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
                  ...
                  <div className="rounded-lg border border-slate-200">
                    <div className="border-b bg-slate-50 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-600">This Round Days</div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="text-left text-xs uppercase tracking-wide text-slate-500">
                        ...
                        <tbody>
                          {digest.workDays.length ? (
                            digest.workDays.map((day, idx) => (
                              <tr key={`${day.showId}-${idx}`} className="border-t border-slate-100 odd:bg-white even:bg-slate-50/40">
                                <td className="px-3 py-2">{day.showName || 'Unknown show'}</td>
                                <td className="px-3 py-2 text-right font-medium">{day.addedDays}</td>
                                <td className="px-3 py-2 text-right">{day.totalDays}</td>
                              </tr>
                            ))
                          ) : (
                            <tr>
                              <td className="px-3 py-3 text-sm text-slate-500" colSpan={3}>No new days</td>
                            </tr>
                          )}
                        </tbody>
                      </table>
              ...
              {history.map((item) => (
                <li key={`${item.digestId}-${item.sentAt}`} className="rounded border border-slate-200 px-3 py-2">
                  ...
                  <p className="text-xs text-slate-500">
                    Sent: {new Date(item.sentAt).toLocaleString()} · Deal Count: {item.orderCount} · Days Sent: {toNum(item.daysSentTotal)}
                  </p>
                  {item.daysSentSummary ? <p className="text-xs text-slate-500">Days Detail: {prettifyDaysSummary(item.daysSentSummary)}</p> : null}
                </li>
              ))}
            </ul>
          </CardContent>
