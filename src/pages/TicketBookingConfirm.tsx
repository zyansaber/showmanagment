...
type ConfirmRequest = { id: string; showId: string; participantIds: string[]; confirmedAt?: string };
...
export default function TicketBookingConfirm() {
  ...
  const confirm = async () => {
    ...
    await dbSet(`ticketBookingConfirmations/${request.id}`, { ...request, confirmedAt } as unknown as Record<string, unknown>);
    setRequest({ ...request, confirmedAt });
