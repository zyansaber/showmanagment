...
export default function Dashboard() {
  ...
  const q1AcceptedOrdersPrevious = q1CompletedShowsCurrent.reduce((sum, show) => sum + parseNumber(show.sales2025), 0);
  ...
  const now = new Date();
  const sameDayLastYearCutoff = new Date(now.getFullYear() - 1, now.getMonth(), now.getDate(), 23, 59, 59, 999);
  const sales2025SamePeriod = orders.reduce((sum, order) => {
    const year = showYearById[order.showId];
    if (year !== 2025 || !isConfirmationOrder(order) || !order.date) return sum;
    const orderDate = new Date(order.date);
    if (Number.isNaN(orderDate.getTime())) return sum;
    return orderDate.getTime() <= sameDayLastYearCutoff.getTime() ? sum + 1 : sum;
  }, 0);
  ...
  const salesVsCards = [
    ...
    },
    {
      ...
      helper: 'Compared with same month/day in 2025',
    },
    {
  ...
  const ringMetrics = [
    { label: 'YTD Target Completion', color: '#ef4444', percent: ytdPercent, radius: 92 },
    { label: '2025 YoY Sales', color: '#6366f1', percent: yoy2025Percent, radius: 72 },
    { label: 'Sales Completion', color: '#10b981', percent: gaugePercent, radius: 52 },
  ];
                ...
                <div className="relative h-[250px] w-[250px]">
                  <svg viewBox="0 0 240 240" className="h-full w-full">
                    {ringMetrics.map((ring) => {
                      const safePercent = Math.min(Math.max(ring.percent, 0), 100);
                      const circumference = 2 * Math.PI * ring.radius;
                      const dashOffset = circumference * (1 - safePercent / 100);
                      const textY = 120 - ring.radius - 8;
                      return (
                        <g key={ring.label}>
                          <circle cx="120" cy="120" r={ring.radius} fill="none" stroke="#e2e8f0" strokeWidth="14" />
                          <circle
                            cx="120"
                            cy="120"
                            r={ring.radius}
                          ...
                          />
                          <text x="120" y={textY} textAnchor="middle" fontSize="12" fontWeight="700" fill={ring.color}>
                            {ring.percent}%
                          </text>
                        </g>
                      );
                ...
                <div className="space-y-3">
                  {ringMetrics.map((ring) => (
                    <div key={ring.label} className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-semibold text-slate-700">{ring.label}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div key={metric.label} className="rounded-lg border p-4 shadow-sm">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        ...
                        {metric.label.includes('Year to Date Target') ? (
                        ...
                        {metric.label.includes('2025 YoY Sales') ? (
        ...
        {showQ1Results ? (
          <CardContent className="space-y-2 text-sm text-slate-700">
            ...
            <p className="text-xs text-slate-500">
              ...
              fallback. Q1 {previousYear} comparison is calculated from the same completed Q1 shows using each show&apos;s
