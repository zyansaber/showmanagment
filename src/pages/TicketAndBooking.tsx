...
type EmailSettings = { serviceId: string; templateId: string; publicKey: string; privateKey: string; subject: string; body: string; confirmationSubject: string; confirmationBody: string };
...
export default function TicketAndBooking() {
  ...
  const notifyAttachmentUpdate = async (record: TicketFile, wasReplacement: boolean) => {
    ...
    const variables = {
      ...
      show_name: show?.name || record.showId,
      file_links: `${record.displayName}: ${record.url}`,
      uploaded_at: new Date(record.replacedAt || record.uploadedAt).toLocaleString(),
      update_type: wasReplacement ? 'updated' : 'uploaded',
    };
    await sendEmailJs(member.email, member.memberName || 'Team member', {
      subject: renderTemplate(settings.subject, variables),
      message: renderTemplate(settings.body, variables),
      show_name: variables.show_name,
      file_links: variables.file_links,
      uploaded_at: variables.uploaded_at,
      update_type: variables.update_type,
    }, settings);
  };
