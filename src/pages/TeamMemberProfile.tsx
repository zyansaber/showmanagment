...
type ProfileSection = 'home' | 'dashboard' | 'showPlan' | 'tickets' | 'messages' | 'personalInfo' | 'snowyCalendar';
...
export default function TeamMemberProfile() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [showsData, teamData, ordersData, filesData, messageData, receiptData] = await Promise.all([
          dbGet('shows'),
          dbGet('teamMembers'),
          dbGet('showOrders'),
  ...
  const menuCards = [
    {
      ...
      title: 'My Dashboard',
      description: 'Orders, confirmations, sales, hours and model range.',
      icon: TrendingUp,
    ...
    },
    {
      ...
      title: 'My Show Plan',
      description: 'Upcoming shows, dates, dealers and attendance time.',
      icon: CalendarDays,
    ...
    },
    {
      ...
      description: 'Open ticket, booking and travel attachments.',
      icon: FileText,
    ...
    },
    {
      ...
      description: 'All shows by calendar, region or list view.',
      icon: CalendarDays,
      color: 'from-cyan-500 to-blue-600',
    },
    {
      id: 'personalInfo' as ProfileSection,
      title: 'Personal Info Edit',
      description: 'Update email, bookmark link, and QR code.',
      icon: UserCog,
    ...
    },
    {
      ...
      description: `${unreadMessages.length} unread message${unreadMessages.length === 1 ? '' : 's'}.`,
      icon: MessageSquare,
      ...
      <div className="mx-auto flex min-h-screen max-w-3xl flex-col px-4 py-5 sm:px-6 sm:py-8">
        <header className="rounded-[2rem] bg-gradient-to-br from-slate-800 to-slate-900 p-5 shadow-2xl ring-1 ring-white/10 sm:p-7">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 rounded-2xl bg-white/10 p-2 text-center">
            ...
            <div><div className="text-xl font-bold">{totalHours}</div><div className="text-[11px] text-slate-300">Hours</div></div>
          </div>
        </header>
            ...
            {upcomingShows.length ? upcomingShows.map((show) => (
              <Card key={show.id} className="rounded-[1.75rem] border-0 shadow-xl">
                <CardContent className="p-5">
                  ...
                  <div className="mt-3 text-sm text-slate-500">Planned hours: {Math.round(((show.id && showDayMap[show.id]) || calculateShowDays(show)) * 8)}</div>
                </CardContent>
              </Card>
          ...
          <main className="space-y-4 py-5 text-slate-950">
            ...
            {calendarView === 'calendar' && (
              <div className="grid gap-3 sm:grid-cols-2">
                {shows.map((show) => (
                  <Card key={show.id} className="rounded-[1.5rem] border-0 shadow-xl">
                    <CardContent className="p-4">
                      <div className="text-sm font-semibold text-blue-600">{formatDate(show.startDate)} - {formatDate(show.finishDate)}</div>
                      <div className="mt-1 font-black text-slate-900">{show.name}</div>
                      <div className="text-sm text-slate-500">{show.siteLocation?.state || 'No State'}</div>
                    </CardContent>
                  </Card>
                ))}
              </div>
            ...
            {calendarView === 'region' && Object.entries(calendarShowsByState).map(([state, stateShows]) => (
              <Card key={state} className="rounded-[1.5rem] border-0 shadow-xl">
                ...
                <CardContent className="space-y-2">
                  {stateShows.map((show) => <div key={show.id} className="rounded-xl bg-slate-50 p-3 text-sm"><b>{show.name}</b><br />{formatDate(show.startDate)} - {formatDate(show.finishDate)}</div>)}
                </CardContent>
              </Card>
            ))}
            {calendarView === 'list' && (
              <Card className="rounded-[1.5rem] border-0 shadow-xl">
                <CardContent className="divide-y p-0">
                  {shows.map((show) => <div key={show.id} className="grid grid-cols-[1fr_90px] gap-2 p-4 text-sm"><span className="font-semibold">{show.name}</span><span className="text-right text-slate-500">{show.siteLocation?.state || '-'}</span><span className="col-span-2 text-slate-500">{formatDate(show.startDate)} - {formatDate(show.finishDate)}</span></div>)}
                </CardContent>
              </Card>
            )}
          </main>
