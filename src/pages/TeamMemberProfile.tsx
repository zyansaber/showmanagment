...
export default function TeamMemberProfile() {
  ...
  const installToHomeScreen = async () => {
    ...
    if (installPrompt) {
      ...
      setInfoMessage(choice.outcome === 'accepted' ? 'Install prompt accepted.' : 'Install prompt dismissed.');
      return;
    }
    if (isIosDevice) {
    ...
    setInfoMessage('If install is not shown, open the browser menu and choose Add to Home screen.');
  };
                    ...
                    {calendarDays.map((day) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex min-h-[4.25rem] flex-col items-center rounded-2xl border p-1.5 text-center shadow-sm transition active:scale-[0.98] sm:min-h-20 ${inMonth ? 'bg-white' : 'bg-slate-50 text-slate-300'} ${dayShows.length ? 'border-blue-300 bg-blue-50 ring-2 ring-blue-100' : 'border-slate-100'}`}
                        >
                          <span className={`text-sm font-black leading-none ${dayShows.length ? 'text-blue-950' : ''}`}>{day.getDate()}</span>
                          {dayStates.length > 0 && (
          ...
          <main className="space-y-4 py-5 text-slate-950">
            <Card className="rounded-[1.75rem] border-0 shadow-xl">
              ...
              <CardContent className="space-y-4">
                ...
                <Button variant="outline" className="w-full gap-2 rounded-2xl" onClick={installToHomeScreen}><Star className="h-4 w-4" />Add to Phone Home Screen</Button>
                {showIosInstallSteps && (
                  <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-slate-700">
                    <div className="font-bold text-slate-900">iPhone / iPad</div>
                    <ol className="mt-2 list-decimal space-y-1 pl-5">
                      <li>Tap the Safari share button.</li>
                      <li>Choose Add to Home Screen.</li>
                      <li>Tap Add to save this team page as an app icon.</li>
                    </ol>
                  </div>
                )}
