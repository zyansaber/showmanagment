...
import { CalendarDays, CheckCircle2, Clock, FileText, MessageSquare, ShoppingCart, TrendingUp, Users } from 'lucide-react';
...
type ProfileSection = 'home' | 'dashboard' | 'showPlan' | 'tickets' | 'messages';
        ...
        <header className="rounded-[2rem] bg-gradient-to-br from-slate-800 to-slate-900 p-5 shadow-2xl ring-1 ring-white/10 sm:p-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-300">{member.email || 'Team member portal'} · no password required</p>
            </div>
            {activeSection !== 'home' && (
              <button
                type="button"
            ...
            <Card className="rounded-[1.75rem] border-0 shadow-xl">
              ...
              <CardContent className="space-y-2">
                {memberFiles.length ? memberFiles.map((file) => (
                  <a key={file.id || file.url} href={file.url} target="_blank" rel="noreferrer" className="block rounded-2xl border bg-white p-4 text-sm font-semibold text-blue-700 hover:bg-blue-50">
                ...
                <Button asChild className="mt-4 w-full rounded-2xl">
                  <Link to="/ticket_and_booking">Upload new files</Link>
                </Button>
              </CardContent>
            </Card>
          </main>
